...
class SlashMenuView {
  ...
  constructor(props: SuggestionProps<SlashCommandItem>) {
    ...
    // Note: `items` is empty on start — the suggestion plugin resolves them
    // asynchronously and delivers them via update(). We only auto-close when
    // the resolved result is empty (see update()).
    this.surface.mount()
  ...
  update(props: SuggestionProps<SlashCommandItem>) {
    ...
    if (props.items.length === 0 && !props.loading) {
      // No matching commands — close the menu instead of showing an empty state.
      this.destroy()
      return
    }
  ...
  onKeyDown(props: SuggestionKeyDownProps): boolean {
    ...
    if (props.event.key === 'ArrowDown') {
      if (!this.surface.getSelectedItem()) return false
    ...
    if (props.event.key === 'Enter') {
      ...
      if (!item) {
        // No command matches — close the menu and let the default Enter
        // behavior (insert newline) proceed.
        this.destroy()
        return false
      }
      this.props.command(item)
    ...
    return [
      Suggestion<SlashCommandItem>({
        ...
        allow: ({ editor, state, range }) => {
          ...
          // Don't trigger on slashes inside URLs or paths ("http://", "https://",
          // "a//b", "a/b/c") — pasting or typing such text would otherwise open
          // an empty slash menu that swallows the Enter key.
          const prev = state.doc.textBetween(Math.max(0, range.from - 1), range.from)
          if (prev === ':' || prev === '/') return false
          const next = state.doc.textBetween(range.to, Math.min(state.doc.content.size, range.to + 1))
